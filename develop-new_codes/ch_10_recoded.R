...
# Save workfile
write_csv(cps,paste0(data_dir,data_out,"earnings_multireg.csv",sep=""))               
...
datasummary_skim( cps %>% dplyr::select( earnwke, uhours, w ) )
datasummary_skim( cps %>% dplyr::select( earnwke, uhours, w )
                      %>% filter( w >= 1 ) )
...
# USE ETABLE
etable( reg , reg2 , reg3 , subtitles = c('wage simple','wage w age','age chck'),
...
etable( reg4 , reg5 , reg6 , fitstat = c('n','r2') ) #, sdBelow = TRUE
...
etable( reg8, reg9, reg10 , fitstat = c('n','r2') ) #, sdBelow = TRUE
...
etable( reg14, reg15, reg16 , fitstat = c('n','r2')  ) #, sdBelow = TRUE
