...
data_in <- paste0(dir,"cases_studies_public/wms-management-survey/clean/")
data_out <- paste0(dir,"textbook_work/ch21/wms-management-survey/")
func <- paste0(dir, "textbook_work/ch00_tech_prep/")
output <- paste0(dir,"textbook_work/ch21/wms-management-survey/output/")
...
# Load in data -------------------------------------------------------
data <- read_dta(paste(data_in,"wms_2004_2010.dta",sep=""))
...
data %>% 
  ...
  summarise(min = min(degree_nm), max = max(degree_nm), n = n())
...
######################################################################
#
# Variation in x, distribution of other variables
#
######################################################################
...
# Save workfile ------------------------------------------------------
write.csv(data, paste0(data_out, "Ch21_wms_workfile.csv"), row.names = F)
...
# Import workfile 
data <- read.csv(paste0(data_out,"Ch21_wms_workfile.csv"))
...
ATE <- summary$management1_Mean - summary$management0_Mean
print(ATE)


# Delete objects
rm(data2, tab, summary, ATE)
...
# SOLUTION #1 (Without replacement) ----------------------------------
# a) Except employment
...
mod_match <- matchit(foundfam_owned ~ 
        ...
        data = data2, method = 'nearest', distance = 'logit', replace=F)
...
mod_match <- matchit(foundfam_owned ~ 
                     degree_nm + degree_nm_sq + factor(compet_moder) + 
                     factor(compet_strong) + factor(industry) + 
                     factor(countrycode) + lnemp, 
                     data = data2, 
                     method = 'nearest', distance = 'logit', replace=F)
...
# Step 3 - Estimate treatment effects (6356 matched)
reg_match <- lm(management ~ foundfam_owned, data = data_match)
...
# Save output
stargazer(reg1, reg2, reg3, out=paste(output,"Ch21_foundfam_reg1_R.tex",sep=""), digits=2, float = F, no.space = T)
...
c1 <- CrossTable(data$foundfam_owned, compet_moder, na.rm=T)
c2 <- CrossTable(data$foundfam_owned, compet_strong, na.rm=T)
