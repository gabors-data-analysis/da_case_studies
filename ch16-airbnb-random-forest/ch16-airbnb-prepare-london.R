...
# Pool bathrooms into meaningful categories
data <- data %>%
  mutate(f_bathroom = cut(
    n_bathrooms,
    breaks = c(0, 1, 2, 10),
    labels = c("0 bathrooms", "1 bathroom", "1+ bathrooms"),
    right = FALSE)
    )

table(data$f_bathroom, useNA = "ifany")

# Pool num of reviews to 3 categories: none, 1-51 and >51
data <- data %>%
  mutate(f_number_of_reviews = cut(
    n_number_of_reviews,
    breaks = c(0,1,51,max(data$n_number_of_reviews) + 1),
    labels= c("0 reviews", "1-50 reviews", "50+ reviews"),
    right = FALSE))

table(data$f_number_of_reviews, useNA = "ifany")

# Pool and categorize the number of minimum nights: 1,2,3, 3+
data <- data %>%
  mutate(f_minimum_nights= cut(
    n_minimum_nights,
    breaks = c(1,2,3,max(data$n_minimum_nights) + 1),
    labels=c("1 night", "2 nights", "2+ nights"),
    right = FALSE))

table(data$f_minimum_nights, useNA = "ifany")
...
data <- data %>%
  mutate(
    ...
    f_bathroom = fct_explicit_na(f_bathroom, na_level = "1 bathroom")
  )
...
table(data$flag_n_number_of_reviews)
