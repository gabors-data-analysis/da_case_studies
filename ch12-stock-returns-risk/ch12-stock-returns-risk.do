...
/*
tempfile workfile
copy "https://osf.io/download/de8uc/" `workfile'
use `workfile', clear
*/
...
generate r_MSFT = 100 * (p_MSFT - p_MSFT[_n-1]) / p_MSFT[_n-1]
label variable r_MSFT "Microsoft returns"
generate r_SP500 = 100 * (p_SP500 - p_SP500[_n-1]) / p_SP500[_n-1]
label variable r_SP500 "SP500 returns"
...
scatter r_MSFT r_SP500, ms(O) mc(navy*0.8) ///
...
generate r_SP500 = 100 * (p_SP500 - p_SP500[_n-1]) / p_SP500[_n-1]
label variable r_SP500 "SP500 returns"
generate dlnp_MSFT = ln(p_MSFT) - ln(p_MSFT[_n-1])
generate dlnp_SP500 = ln(p_SP500) - ln(p_SP500[_n-1])
...
regress r_MSFT r_SP500, robust
outreg2 using "${output}/ch12-table-3-stocks-reg.tex", tex(frag) dec(4) 2aster label ///
	ctitle("MSFT returns, monthly, pct change") replace
...
regress r_MSFT r_SP500, robust
outreg2 using "${output}/ch12-table-3-stocks-reg.tex", tex(frag) dec(4) 2aster label ///
	ctitle("MSFT returns, monthly, log change") append
...
regress r_MSFT r_SP500, robust
outreg2 using "${output}/ch12-table-3-stocks-reg.tex", tex(frag) dec(4) 2aster label ///
	ctitle("MSFT returns, daily, pct change") append
replace r_MSFT = dlnp_MSFT  /* To have the estimates in the same row in the table */
replace r_SP500 = dlnp_SP500  /* To have the estimates in the same row in the table */
regress r_MSFT r_SP500, robust
outreg2 using "${output}/ch12-table-3-stocks-reg.tex", tex(frag) dec(4) 2aster label ///
	ctitle("MSFT returns, daily, log change") append
